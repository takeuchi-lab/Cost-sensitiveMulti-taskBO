...
##### argment #####
parser = argparse.ArgumentParser(description="[Multi-task BO for GB structure]")
...
##### output regret log #####
if args.cost:
	outDIR = "./log/"+args.exp_name+"/CMB_log"
else:
	outDIR = "./log/"+args.exp_name+"/MB_log"
...
for t in (range(T)):
	...
	if t%2 == 0:
		...
		with open(outDIR+"/log_exp_tmp_"+str("%03d"%args.init_No)+".pickle","wb") as f:
			pickle.dump(log_set,f)
